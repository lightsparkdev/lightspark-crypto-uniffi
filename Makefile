...
build_android_arm64: android_abi = aarch64-linux-android
build_android_arm64:
	cargo build --profile release-smaller --target=$(android_abi)

build_android_x86: android_abi = x86_64-linux-android
build_android_x86:
	cargo build --profile release-smaller --target=$(android_abi)

build_android_arm7: android_abi = armv7a-linux-androideabi
build_android_arm7:
	cargo build --profile release-smaller --target=armv7-linux-androideabi

copy_android_libs:
	mkdir -p lightspark-crypto-kotlin/jniLibs/arm64-v8a
	mkdir -p lightspark-crypto-kotlin/jniLibs/armeabi-v7a
	mkdir -p lightspark-crypto-kotlin/jniLibs/x86_64
	cp -r target/aarch64-linux-android/release-smaller/liblightspark_crypto.so lightspark-crypto-kotlin/jniLibs/arm64-v8a/libuniffi_lightspark_crypto.so
	cp -r target/armv7-linux-androideabi/release-smaller/liblightspark_crypto.so lightspark-crypto-kotlin/jniLibs/armeabi-v7a/libuniffi_lightspark_crypto.so
	cp -r target/x86_64-linux-android/release-smaller/liblightspark_crypto.so lightspark-crypto-kotlin/jniLibs/x86_64/libuniffi_lightspark_crypto.so

build-android-targets: build_android_arm64 build_android_x86 build_android_arm7